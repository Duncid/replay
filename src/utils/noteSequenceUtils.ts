...
export const ABC_TO_SOLFEGE_MAP = Object.freeze(
  Object.fromEntries(
    NOTE_NAMES.map((note, index) => [note, SOLFEGE_NAMES[index]]),
  ),
);
export const SOLFEGE_TO_ABC_MAP = Object.freeze(
  Object.fromEntries(
    SOLFEGE_NAMES.map((solfege, index) => [solfege, NOTE_NAMES[index]]),
  ),
);
 ...
 */
export function createEmptyNoteSequence(
  qpm: number = DEFAULT_QPM,
  timeSignature = "4/4",
): NoteSequence {
 ...
 */
export function noteSequenceToAbc(
  sequence: NoteSequence,
  title?: string,
): string {
  ...
  sortedKeys.forEach((key) => {
    ...
    if (notes.length === 1) {
      ...
      const minDuration = Math.min(
        ...notes.map((n) => n.endTime - n.startTime),
      );
      const durationInBeats = secondsToBeats(minDuration, qpm);
      const abcChord =
        "[" + notes.map((n) => pitchToAbcNote(n.pitch)).join("") + "]";
 ...
 */
export function validateNoteSequence(sequence: NoteSequence): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  sequence.notes.forEach((note, i) => {
    if (!isValidPitch(note.pitch)) {
      errors.push(
        `Note ${i}: pitch ${note.pitch} (${midiToNoteName(note.pitch)}) is outside valid range C3-C6`,
      );
    }
    if (note.startTime < 0) {
    ...
    if (note.endTime <= note.startTime) {
      errors.push(
        `Note ${i}: endTime (${note.endTime}) must be greater than startTime (${note.startTime})`,
      );
    }
    if (note.velocity < 0 || note.velocity > 1) {
      errors.push(
        `Note ${i}: velocity ${note.velocity} must be between 0 and 1`,
      );
    }
  });
 ...
 */
export function abcToNoteSequence(
  abc: string,
  qpm: number = DEFAULT_QPM,
): NoteSequence {
...
function abcNoteToPitch(
  accidental: string,
  noteLetter: string,
  octaveMarkers: string,
): number {
